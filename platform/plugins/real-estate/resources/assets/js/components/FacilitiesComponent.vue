    ...
    <div>
        <div class="form-group mb-3" v-for="(item, index) in items">
            <div class="row">
                <div class="col-md-3 col-sm-5">
                    <div class="form-group mb-3">
                        <div class="ui-select-wrapper">
                            <select :name="'facilities[' + (item.id ? item.id : 0) + '][id]'" v-model="item.id"
                                    class="select-search-full ui-select">
                                <option value="">{{ __('select_facility') }}</option>
                                <option :value="facility.id" @change="removeSelectedItem(facility.id)"
                                        v-for="facility in facilities">{{ facility.name }}
                                </option>
                            </select>
                ...
                <div class="col-md-3 col-sm-5">
                    <div class="form-group mb-3">
                        <input type="text" :name="'facilities[' + (item.id ? item.id : 0) + '][distance]'"
                               v-model="item.distance" class="form-control"
